...
Commit: `28d1f791` `af145d8f` `a40a43f2` `97f3ac1c` `3f0d0d07`
...
## 2026-07-18 - export-deliverables-goal-2026-07-18

Commit: pending

Prompt ID: `export-deliverables-goal-2026-07-18`

Objective:

- Replace honest static export gating with real, verifiable PDF, PNG and PPTX deliverables.
- Keep export success tied to generated bytes, revision metadata, filters and allowed download scope.

Files changed:

- `types/export.ts`
- `lib/export/contracts.ts`
- `lib/export/binary.ts`
- `lib/export/renderers.ts`
...
- `components/public-share-page.tsx`
- `components/presentation/presentation-builder.tsx`
- `tests/export-deliverables.test.ts`
- Existing capability, CTA and E2E tests updated for real static exports.

Architecture notes:

- Added Zod contracts for `ExportRequest`, `ExportTarget`, `ExportFormat`, `ExportScope`, `ExportStatus`, `ExportResult` and `ExportError`.
- Added export job state transitions: `queued`, `rendering`, `generating`, `ready` and `failed`, surfaced in dashboard/share/presentation UI.
- PDF generation writes a valid PDF with title, source, timestamp, dashboard revision, filters and widget result summaries.
- PNG generation writes deterministic valid PNG bytes with fixed dimensions and metadata chunks; dashboard PNG is 1200x800 and slide PNG is 1280x720.
- PPTX generation writes a real Open XML package with ordered slides, editable text, evidence/revision metadata and speaker-note text represented inside slides.
- Exports use `DashboardSpec`, `PresentationSpec`, view state, filters and queryable rows/snapshots. They do not report success before `Blob` creation and download.
- App import now rehydrates the queryable dataset after persistence so exports use the canonical dataset version returned by data-access.
- Public share export rejects payloads without `allowDownload`/`export_snapshot`; UI disables downloads when the link forbids them.

Validation:

- `npm.cmd run typecheck`: passed during implementation.
- `npm.cmd run lint`: passed during implementation.
- `npm.cmd run test -- tests/export-deliverables.test.ts tests/cta-capabilities.test.tsx tests/share.test.ts tests/public-share-page.test.tsx`: passed, 4 files and 18 tests.
- `npm.cmd run test:e2e -- tests/e2e/capability-ctas.spec.ts tests/e2e/product-workflow.spec.ts`: passed after sandbox escalation, 5 Chromium tests.
- Full validation is recorded in `docs/checkpoints/export-deliverables-goal-2026-07-18.md`.
...
- No SQL migration added.
- No dependency added, so `npm ci` was not required.
- No environment variable added.

Security/privacy notes:

- No service-role key, Supabase secret, `AI_API_KEY`, `.env`, generated build artifact or test report file is part of the intended change.
- Public share exports operate on published widget snapshots/results and explicit download scope, not raw source rows.
